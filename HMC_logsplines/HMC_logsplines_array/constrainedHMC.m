function [q0, qs, stats] = constrainedHMC(q0,nllFunc,conFunc,Mfunc,N,L,h,opts)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% This code is copyright 2012 Marcus A Brubaker and is distributed freely for academic and personal use only.  
    %%% Persons interested in commercial use of this code should contact Marcus A Brubaker.  There is no warranty 
    %%% implied or personal liability assumed by any of the authors.  By downloading this code, you accept these terms.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             disp(str);
        ...
        if norm(c) > sqrt(1e-4) || isempty(p) %|| sum(abs(real(q))>1)~=0 %||...
                %norm(dU)>1e+04
            H = inf;
            deltaU = inf;
            deltaH = inf;
...
%                     break;
        ...
        if isempty(nllGradFunc)
            if sum(isnan(q))>0
               disp("There are missing values in the parameter vector!"); 
            end
            [U,dU] = nllFunc(q);
            stats.nllFuncCalls = stats.nllFuncCalls + 1;
            stats.nllGradFuncCalls = stats.nllGradFuncCalls + 1;

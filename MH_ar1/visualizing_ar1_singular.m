...
%     end
%     rhos(i) = rho_av/59;
    rhos(i) = log(sigma(1,2))-log(sigma(1,1));
    s2s(i) = sigma(1,1)*(1 - (sigma(1,2)/sigma(1,1)).^2);
    s_diagonal_terms(i) = sigma(1,1);
end

histogram(exp(rhos),'Normalization','probability');
hold on
...
histogram(sqrt(s2s),'Normalization','probability');
hold on
...
histogram(s_diagonal_terms,'Normalization','probability');
